...
const applyDefaults = ctx => extend({}, def, ctx, {
...
export default ((dest, ctx) => {
  ...
  return Promise.all([copy(path.resolve(__dirname, '../assets'), path.resolve(dest, 'assets')).then(shortcutIcon(dest, ctx)), renderFile(index, ctx).then(html => minify(html, { collapseWhitespace: true })).then(html => writeFile(path.resolve(dest, 'index.html'), html))]);
})
