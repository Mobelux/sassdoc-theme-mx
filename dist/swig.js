...
const displayAsType = input => input.split('|').map(x => x.trim()).map(swigFilters.capitalize).join('</code> or <code>');

const yiq = ([red, green, blue]) => (red * 299 + green * 587 + blue * 114) / 1000;

const yiqContrast = rgb => yiq(rgb) >= 128 ? '#000' : '#fff';

const getChannel = (start, hex) => parseInt(hex.substr(start, 2), 16);

const hexToRgb = hex => [0, 2, 4].map(x => getChannel(x, hex));

const colorToHex = color => chroma(color).hex().substr(1);
 ...
 */
const maybeYiqContrast = color => isColor(color) ? yiqContrast(hexToRgb(colorToHex(color))) : '#000';
